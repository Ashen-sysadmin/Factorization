...
public class ColossusController extends EntityFz implements IBossDisplayData, IDCController {
    ...
    public final StateMachineExecutor ai_controller = new StateMachineExecutor(
        this,
        "tech",
        Technique.STATE_MACHINE_ENTRY);
    boolean setup = false;
    ...
    public ColossusController(World world, LimbInfo[] limbInfo, int arm_size, int arm_length, int leg_size,
        int leg_length, int width) {
        this(world);
        this.limbs = limbInfo;
    ...
    protected void calcLimbParity() {
        ...
        discover_limb_parities: for (LimbInfo li : limbs) {
            ...
            switch (li.type) {
                default:
                    continue discover_limb_parities;
                case ARM:
    ...
    public void putData(DataHelper data) throws IOException {
        int limb_count = data.as(Share.PRIVATE, "limbCount")
            .putInt(limbs == null ? 0 : limbs.length);
        if (data.isReader()) {
        ...
        leg_size = data.as(Share.VISIBLE, "legSize")
            .putInt(leg_size);
        arm_size = data.as(Share.VISIBLE, "armSize")
            .putInt(arm_size);
        arm_length = data.as(Share.VISIBLE, "armLength")
            .putInt(arm_length);
        body_width = data.as(Share.VISIBLE, "bodyWidth")
            .putInt(body_width);
        if (data.isReader() && data.isNBT() && body_width < 1) {
            body_width = 5; // Well, whatever!
        }
        home = data.as(Share.PRIVATE, "home")
            .putIDS(home);
        if (data.as(Share.PRIVATE, "has_path_target")
            .putBoolean(path_target != null)) {
            path_target = data.as(Share.PRIVATE, "path_target")
                .putIDS(path_target);
        }
        setTotalCracks(
            data.as(Share.VISIBLE, "cracks")
                .putInt(getTotalCracks()));
        setDestroyedCracks(
            data.as(Share.VISIBLE, "broken")
                .putInt(getDestroyedCracks()));
        turningDirection = data.as(Share.PRIVATE, "turningDirection")
            .putInt(turningDirection);
        target_changed = data.as(Share.PRIVATE, "targetChanged")
            .putBoolean(target_changed);
        walked = data.as(Share.PRIVATE, "walked")
            .putDouble(walked);
        target_count = data.as(Share.PRIVATE, "target_count")
            .putInt(target_count);
        target_y = data.as(Share.PRIVATE, "target_y")
            .putDouble(target_y);
        last_step_direction = data.as(Share.PRIVATE, "last_step_direction")
            .putInt(last_step_direction);
        been_hurt = data.as(Share.PRIVATE, "been_hurt")
            .putBoolean(been_hurt);
        spin_direction = data.as(Share.PRIVATE, "spin_dir")
            .putEnum(spin_direction);
        confused = data.as(Share.PRIVATE, "confused")
            .putBoolean(confused);
    }

    <E extends Enum<E>> EnumSet<E> putEnumSet(DataHelper data, String prefix, EnumSet<E> set, Class<E> elementType)
        throws IOException {
        if (!data.isNBT()) return set;
        if (data.isWriter()) {
            for (E e : set) {
                data.asSameShare(prefix + e.name())
                    .putBoolean(true);
            }
            return set;
        } else {
            ...
            for (E e : elementType.getEnumConstants()) {
                if (data.asSameShare(prefix + e.name())
                    .putBoolean(false)) {
                    ret.add(e);
                }
            }
