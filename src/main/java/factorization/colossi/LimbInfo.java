...
class LimbInfo {
    ...
    void putData(DataHelper data, int index) throws IOException {
        type = data.as(Share.VISIBLE, "limbType" + index)
            .putEnum(type);
        side = data.as(Share.VISIBLE, "limbSide" + index)
            .putEnum(side);
        parity = data.as(Share.VISIBLE, "limbParity" + index)
            .putByte(parity);
        length = data.as(Share.VISIBLE, "limbLength" + index)
            .putInt(length);
        idc = data.as(Share.VISIBLE, "entUuid" + index)
            .putIDS(idc);
        lastTurnDirection = data.as(Share.VISIBLE, "lastTurnDir" + index)
            .putByte(lastTurnDirection);
    }
    ...
    /**
     * Orders this limb to move to a target rotation. The rotation time is dependent on the rotational distance, the
     * limb size, and power.
     * 
     * @param rot   target rotation
     * @param power How much (completely fake) power to use. Higher amounts of power make the limb move faster. 1 is the
     *              default value.
     */
