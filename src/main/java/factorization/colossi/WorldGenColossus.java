...
public class WorldGenColossus implements IWorldGenerator {
    ...
    static {
        if (GENERATION_START_X > GENERATION_SPACING || GENERATION_START_Z > GENERATION_SPACING) {
            throw new IllegalArgumentException(
                "colossus spacing must be at least " + Math.max(GENERATION_START_X, GENERATION_START_Z));
        }
    }
    ...
    public static ArrayList<Coord> getCandidatesNear(final Coord player, int chunkSearchDistance, boolean forceLoad)
        ...
        ChunkCoordIntPair chunkAt = player.getChunk()
    ...
    public void injectNoiseSmoothers(InitNoiseGensEvent event) {
        ...
        for (int noise_index : target_noises) {
            ...
            event.newNoiseGens[noise_index] = new SmoothNoiseNearColossi(
                noise_index,
                (NoiseGeneratorOctaves) parentGenerator);
        }
    }
    ...
    static class SmoothNoiseNearColossi extends NoiseGeneratorOctaves {
        ...
        @Override
        public double[] generateNoiseOctaves(double[] noiseOut, int chunkXTimes4, int noiseStartY, int chunkZTimes4,
            int noiseSizeX, int noiseSizeY, int noiseSizeZ, double scaleX, double scaleY, double scaleZ) {
            ...
            double[] ret = parent.generateNoiseOctaves(
                noiseOut,
                chunkXTimes4,
                noiseStartY,
                chunkZTimes4,
    ...
    public static boolean genOnWorld(World world) {
        return world.getWorldInfo()
            .isMapFeaturesEnabled() && world.provider.isSurfaceWorld()
            && FzConfig.gen_colossi
            && !dimensionBlacklist.containsKey(world.provider.dimensionId);
    }

    static Type[] forbiddenBiomeTypes = new Type[] { SPOOKY, NETHER, END, MAGICAL, WATER, RIVER, OCEAN, BEACH };
    ...
    @Override
    public void generate(Random worldRandom, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
    ...
    public void cancelDecorations(Decorate event) {
        ...
        if (type == Decorate.EventType.BIG_SHROOM || type == Decorate.EventType.LAKE
            || type == Decorate.EventType.PUMPKIN
            || type == Decorate.EventType.TREE) {
