...
public class WrappedPacketFromServer extends WrappedPacket {
    ...
    public void processPacket(INetHandler netHandler) {
        ...
        if (Hammer.log_client_chunking
            && (wrapped instanceof S21PacketChunkData || wrapped instanceof S26PacketMapChunkBulk)) {
        ...
        if (wrapped instanceof FMLProxyPacket) {
            ...
            NetworkDispatcher dispatcher = nhpc.getNetworkManager()
                .channel()
        ...
        if (needReset) {
            ...
            // Consider the multisending of { vanilla packet, FML packet } from a { integrated SSP server, SMP server,
            // LAN-hosting SSP server }.
            ...
            // FML packets & integrated: **The byte buffer gets read in the client thread multiple times; the ByteBuf is
            // sad.**
            // FML packets & SMP: Given the above issue, you'd think it'd be a problem, but I guess it's copying the
            // byte buffer rather than reading
            // with a similar mechanism.
            // FML packets & LAN: Same issue with integrated applies. Remote clients anticipated to have the same
            // immunity.
            //
            ...
            // But unfortunately peeps send packets incorrectly by player-distance instead of properly using the
            // subscription,
