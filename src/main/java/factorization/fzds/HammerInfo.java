...
public class HammerInfo {
    ...
    void setConfigFile(File f) {
        ...
        dimension_slice_dimid = channelConfig
            .getInt("FzdsDimension", "Hammer", dimension_slice_dimid, Integer.MIN_VALUE, Integer.MAX_VALUE, rem);
        saveChannelConfig();
    }
    ...
    boolean isFree(String name, int val) {
        for (String categoryName : channelConfig.getCategoryNames()) {
            ...
            if (cat.getQualifiedName()
                .equals(name)) {
                if (channelConfig.get(categoryName, "channel", val)
                    .getInt() == val) {
                    return true;
                }
                // Uhm.
            ...
            int here_chan = channelConfig.get(categoryName, "channel", -1)
                .getInt();
            if (here_chan == val) {
                return false;
            }
    ...
    public int makeChannelFor(String modName, String channelName, int default_channel_id, int padding, String comment) {
        ...
        if (collision) {
            Core.logFine(
                "Default channel ID for %s (%s) was already taken, using %s",
                modCategory,
                default_channel_id,
                channel_id);
        ...
        int channelRet = channelConfig.get(modCategory, "channel", default_channel_id)
            .getInt();
        if (!isFree(modCategory, channelRet)) {
            ...
            Core.logWarning(
                "Saved channel ID for %s (%s) conflicted, changing to %s",
                modCategory,
                channelRet,
                fixedChannel);
            channelRet = fixedChannel;
            channelConfig.get(modCategory, "channel", channelRet)
                .set(channelRet);
        }
        padding = channelConfig.get(modCategory, "padding", padding)
            .getInt();
    ...
    Coord takeCell(int channel, DeltaCoord size) {
        ...
        Coord ret = new Coord(
            ...
            start,
            16,
            roundToChunk(channel * Hammer.channelWidth));
