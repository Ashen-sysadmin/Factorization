...
public class TransferLib {
    ...
    public static final int SET_SNEAKY = 0, SET_SNEAKY_NO_LIGHTING_UPDATES = 1, SET_ISREMOTE = 2, SET_DIRECT = 3,
        SET_VANILLA_FLAGS = 4, SET_CHUNKY = 5;
        ...
        switch (use_method) {
            ...
            case SET_SNEAKY_NO_LIGHTING_UPDATES: {
                ...
                if (extendedblockstorage == null) {
                    ...
                    extendedblockstorage = storageArrays[c.y >> 4] = new ExtendedBlockStorage(
                        c.y >> 4 << 4,
    ...
    public static TileEntity move(Coord src, Coord dest, boolean wipeSrc, boolean overwriteDestination, int setMethod) {
        ...
        {
            List<NextTickListEntry> pendingTicks = (List<NextTickListEntry>) src.w
                .getPendingBlockUpdates(src.getChunk(), false);
            if (pendingTicks != null) {
                for (NextTickListEntry tick : pendingTicks) {
                    if (tick.xCoord == src.x && tick.yCoord == src.y && tick.zCoord == src.z) {
                        block_tick_time = tick.scheduledTime - src.w.getWorldInfo()
                            .getWorldTotalTime();
        ...
        if (block_tick_time > -1) {
            dest.w.scheduleBlockUpdateWithPriority(
                dest.x,
                dest.y,
                dest.z,
                id,
