...
class ShadowRenderGlobal implements IWorldAccess {
    ...
    void markBlocksForUpdate(int lx, int ly, int lz, int hx, int hy, int hz) {
        ...
        for (IDeltaChunk idc : DeltaChunk.getSlicesInRange(realClientWorld, lx, ly, lz, hx, hy, hz)) {
            ...
            if (NumUtil.intersect(near.x, far.x, lx, hx) && NumUtil.intersect(near.y, far.y, ly, hy)
    ...
    @Override
    public void playSoundToNearExcept(EntityPlayer entityplayer, String sound, double x, double y, double z,
    ...
    public void playRecord(String recordName, int x, int y, int z) {
        ...
        HammerClientProxy.getRealRenderGlobal()
            .playRecord(recordName, (int) realCoords.xCoord, (int) realCoords.yCoord, (int) realCoords.zCoord);
    }
    ...
    public void destroyBlockPartially(int breakerId, int blockX, int blockY, int blockZ, int damageProgress) {
        ...
        if (destroyblockprogress == null || destroyblockprogress.getPartialBlockX() != blockX
    ...
    public void removeStaleDamage() {
        ...
        Iterator<DestroyBlockProgress> iterator = damagedBlocks.values()
            .iterator();
