...
public class Hammer {
    ...
    final String[] Lore = new String[] { "At twilight's end, the shadow's crossed,",
        "A new world birthed, the elder lost.", "Yet on the morn we wake to find", "That mem'ry left so far behind.",
        "To deafened ears we ask, unseen,", "“Which is life and which the dream?”" // -- Aaron Diaz,
                                                                                   // http://dresdencodak.com/2006/03/02/zhuangzi/
    };
    ...
    public static World worldClient = null; // This is actually a WorldClient that is actually
                                            // HammerClientProxy.HammerWorldClient
    public static double DSE_ChunkUpdateRangeSquared = Math.pow(16 * 8, 2); // This is actually set when the server
                                                                            // starts
    ...
    public static boolean log_client_chunking = Boolean
        .parseBoolean(System.getProperty("fzds.logClientChunks", "false"));
    ...
    public void setup(FMLPreInitializationEvent event) {
        ...
        int client_despawn_distance = 16 * 10; // NORELEASE: This wants for a config setting. "How far away the a client
                                               // must be from a DSE before the server will tell the client to forget
                                               // about it (eg, client side despawn)."
        ...
        fzds_command_channel = hammerInfo.makeChannelFor(
            Core.modId,
            "fzdscmd",
            fzds_command_channel,
            -1,
            "This channel is used for Slices created using the /fzds command");
        ...
        if (log_client_chunking && FMLCommonHandler.instance()
            .getSide() == Side.CLIENT) {
    ...
    public void serverStart(FMLServerStartingEvent event) {
        ...
        if (!(hammerWorld.provider instanceof HammerWorldProvider)) {
            throw new RuntimeException(
                "Expected HammerWorldProvider for HammerWorld; is there a dimension ID conflict? Actual provider: "
                    + hammerWorld.provider.getClass());
        }
        hammerWorld.addWorldAccess(new ServerShadowWorldAccess());
        int view_distance = MinecraftServer.getServer()
            .getConfigurationManager()
            .getViewDistance();
        // the undeobfed method comes after "isPlayerWatchingChunk", also in uses of
        // ServerConfigurationManager.getViewDistance()
