...
public abstract class IDeltaChunk extends EntityFz {
    ...
    /**
     * Sets the parent. The parent can be null. Reparenting probably isn't synchronized, so set it before spawning in
     * the IDC.
     * Attatches the slice to its parent. When the parent translates & rotates, the child follows, *and* does the same
     * physics calculations as if it had native motion.
     ...
     *
     * The return value of {@link factorization.fzds.interfaces.IDeltaChunk#getParentJoint} is determined from the
     * positioning of the parent and this child.
     *
    ...
    /**
     * Sets the rotational velocity (omega), relative to the world. The default Quaternion represents zero angular
     * velocity.
     * The IDC's Entity position is the origin of the rotation, and the getRotationalCenterOffset defines the center of
     * mass.
     * 
     ...
     * Consider a carousel with a grandfather clock standing on it. The cabinet and the carousel would be represented
     * with 1 IDC,
     * and the hands of the clock would be each represented with an IDC. Let the carousel be still and at its default
     * position,
     * with the clock facing NORTH. When it is 12 o'clock, the hand will have zero rotation, and when it's 6 o'clock,
     * the hand will
     * have half a turn around the NORTH axis. Using this method to order rotations between 6 o'clock and 12 o'clock
     * will have the
     * natural, desired effect.
     * 
     * However, using setRotation or setRotationalVelocity can result in the hand not being flush with the face of the
     * clock, since
    ...
    /**
     * @return how much time remains on the rotation. The return value is undefined if !hasOrderedRotation(), so be sure
     *         to check that first.
     */
    ...
    /**
     * @param newOffset The new rotational offset. This is relative to getCorner(). Calling this method after the entity
     *                  has been
     *                  spawned is untested.
     */
     ...
     * Helper function.
     * 
     * @param newCenter takes a real-world vector, and makes it the center of rotation, preserving the apparent position
     *                  of the slice. (The IDC's actual position will change, however.)
     * 
    ...
    /**
     * Enables a {@link DeltaCapability}. This should be done before spawning the entity, as clients will not get
     * updates to this.
     * 
     * @param cap A {@link DeltaCapability}
     * @return this
    ...
    /**
     * Disables a {@link DeltaCapability}. This should be done before spawning the entity, as clients will not get
     * updates to this.
     * 
     * @param cap A {@link DeltaCapability}
     * @return this
    ...
    public void loadUsualCapabilities() {
        ...
        for (DeltaCapability cap : new DeltaCapability[] { COLLIDE, MOVE, ROTATE, DRAG, REMOVE_EXTERIOR_ENTITIES,
            INTERACT, BLOCK_PLACE, BLOCK_MINE, REMOVE_ITEM_ENTITIES, ENTITY_PHYSICS, }) {
            permit(cap);
        }
    }
    ...
    public String toString() {
        ...
        if (partName != null && !partName.isEmpty()) {
            ...
            ret = super.toString() + " - from "
                + getCorner()
                + "  to  "
                + getFarCorner()
                + "   center at "
                + getRotationalCenterOffset();
        }
        if (getParent() != null) {
    ...
    /**
     * @param controller The controller responsible for this IDC. The IDC will not track this value across
     *                   serializations; it is the controller's responsibility to set it.
     */
