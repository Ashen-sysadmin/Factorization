...
public class ASMTransformer implements IClassTransformer {
    ...
    public static boolean dev_environ = Launch.blackboard != null
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        // Sorted by beauty!
        {
            ...
            if (transformedName.equals("net.minecraft.block.Block")) {
                return applyTransform(
                    basicClass,
                    new AbstractAsmMethodTransform.Append(
                        name,
                        transformedName,
                        "func_149723_a",
...

        {
            ...
            if (transformedName.equals("net.minecraft.client.Minecraft") && HammerEnabled.ENABLED) {
                return applyTransform(
                    basicClass,
                    new AbstractAsmMethodTransform.Prepend(name, transformedName, "func_147116_af", "func_147116_af"), // "attack
                                                                                                                       // key
                                                                                                                       // pressed"
                                                                                                                       // function
                                                                                                                       // (first
                                                                                                                       ...
                                                                                                                       // name
                                                                                                                       // clickMouse
                    new AbstractAsmMethodTransform.Prepend(name, transformedName, "func_147121_ag", "func_147121_ag") // "use
                                                                                                                      // key
                                                                                                                      // pressed"
                                                                                                                      // function,
                                                                                                                      // MCPBot
            ...
            if (transformedName.equals("net.minecraft.client.renderer.EntityRenderer") && HammerEnabled.ENABLED) {
                try {
                    ...
                    log(
                        "This is lame, but ignorable: the camera will pass through solid FZDS blocks in 3rd-person view.");
                    e.printStackTrace();
                }
            }
            // Add "Universal Colliders". Adds a list of entities to the chunk that are added to every collision query.
            // The alternative to this is setting World.MAX_ENTITY_RADIUS to a large value, and putting
            // collodier-entities everywhere, which is slow & ugly
            if (transformedName.equals("net.minecraft.world.chunk.Chunk") && HammerEnabled.ENABLED) {
                return applyTransform(
                    basicClass,
                    new AbstractAsmClassTransform.Mixin(
                        "factorization.coremodhooks.MixinExtraChunkData",
                        "Lfactorization/coremodhooks/MixinExtraChunkData;"),
                    new AbstractAsmMethodTransform.Append(
                        name,
                        transformedName,
                        "func_76588_a",
            ...
            if (transformedName.equals("net.minecraft.world.World") && HammerEnabled.ENABLED) {
                try {
                    ...
                    log("WARNING: Transformation to " + transformedName + " failed!");
                    log(
                        "This is lame, but (kind-of?) ignorable: you may be kicked for flying when standing on FZDS blocks ");
                    e.printStackTrace();
                }
            }
            // (This... might not be entirely necessary. I didn't test this problem enough!)
            // Something about limiting the velocity of an entity when it's being influenced by multiple IDCs moving in
            // the same direction...
            ...
            if (transformedName.equals("net.minecraft.entity.Entity") && HammerEnabled.ENABLED) {
                return applyTransform(
                    basicClass,
                    new AbstractAsmClassTransform.Mixin(
                        "factorization.coremodhooks.MixinEntityKinematicsTracker",
                        "Lfactorization/coremodhooks/MixinEntityKinematicsTracker;"));
            }
            ...
            if (transformedName.equals("net.minecraft.world.Explosion") && HammerEnabled.ENABLED) {
                return applyTransform(
                    ...
                    new AbstractAsmMethodTransform.MutateCall(name, transformedName, "func_77278_a", "doExplosionA")
                        // find(String owner, String srg_name, String mcp_name, String notch_name, String
                        // find_notch_desc)
            ...
            if (transformedName.equals("net.minecraft.client.multiplayer.WorldClient") && HammerEnabled.ENABLED) {
                try {
                    ...
                    if (FMLClientHandler.instance()
                        .hasOptifine()) {
                        log(
                            "WorldClient FZDS patch failed! This is due to the presence of OptiFine. Minimap mods may crash, however - you'll need to choose one or the other if so.");
                        e.printStackTrace();
                    } else {
        ...
        }
        {
            ...
            if (transformedName.equals("net.minecraft.block.BlockRailBase")
                && !Boolean.parseBoolean(System.getProperty("fz.disableRailStairs"))) {
                return applyTransform(
                    basicClass,
                    new AbstractAsmClassTransform.Mixin(
                        "factorization.coremodhooks.MixinRailStairs",
                        "Lfactorization/coremodhooks/MixinRailStairs;"));
            }
        }
        {
    ...
    byte[] applyTransform(byte[] basicClass, AbstractAsmClassTransform[] classChanges,
        AbstractAsmMethodTransform... changes) {
        ...
        for (AbstractAsmMethodTransform change : changes) {
            if (!change.satisfied) {
                throw new RuntimeException(
                    "Unable to find method " + cn.name + "." + change.srgName + " (" + change.mcpName + ")");
            }
        }
