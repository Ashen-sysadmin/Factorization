...
abstract public class FactorizationBlockRender implements ICoord {
    ...
    protected void renderPart(RenderBlocks rb, IIcon texture, float b1, float b2, float b3, float b4, float b5,
        float b6) {
    ...
    protected void vertex(WireRenderingCube rc, float x, float y, float z, float u, float v) {
        // all units are in texels; center of the cube is the origin. Or, like... not the center but the texel that's
        // (8,8,8) away from the corner is.
        // u & v are in texels
        ...
        Tessellator.instance.addVertexWithUV(
            ...
            this.y + 0.5 + y / 16F,
            this.z + 0.5 + z / 16F,
            wire.getInterpolatedU(u),
            wire.getInterpolatedV(v));
    }
