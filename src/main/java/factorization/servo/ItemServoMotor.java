...
public class ItemServoMotor extends ItemCraftingComponent {
    ...
    @Override
    public boolean onItemUse(ItemStack is, EntityPlayer player, World w, int x, int y, int z, int side, float vecX,
    ...
    @Override
    public boolean onItemUseFirst(ItemStack is, EntityPlayer player, World w, int x, int y, int z, int side, float hitX,
        ...
        for (ForgeDirection fd : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (motor.motionHandler.validDirection(fd, false)) {
                FzOrientation t = FzOrientation.fromDirection(fd)
                    .pointTopTo(top);
        ...
        final Vec3 vP = Vec3.createVectorHelper(hitX, hitY, hitZ)
            .normalize();
        Collections.sort(valid, new Comparator<FzOrientation>() {
            ...
            public int compare(FzOrientation a, FzOrientation b) {
                double dpA = vP
                    .dotProduct(Vec3.createVectorHelper(a.facing.offsetX, a.facing.offsetY, a.facing.offsetZ));
                double dpB = vP
                    .dotProduct(Vec3.createVectorHelper(b.facing.offsetX, b.facing.offsetY, b.facing.offsetZ));
