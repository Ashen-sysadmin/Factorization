...
public class FactorizationOreProcessingHandler {
    ...
    void addProcessingFront(OreType oreType, ItemStack ore, ItemStack ingot) {
        ...
        TileEntitySlagFurnace.SlagRecipes.register(dirty, 1.1F, ingot, 0.2F, Blocks.dirt); // Or it could output reduced
                                                                                           // chunks instead.
    ...
    void handleNewOre(String oreClass, ItemStack ore) {
        ...
        if (ItemOreProcessing.OD_ores.contains(oreClass)) {
            ...
            if (ingot == null) {
                ingot = FurnaceRecipes.smelting()
                    .getSmeltingResult(ore);
            }
            ...
            if (oreType == OreType.GALENA) {
                ...
                TileEntitySlagFurnace.SlagRecipes.register(
                    ore,
                    1.2F,
                    new ItemStack(Core.registry.lead_ingot),
                    1F,
                    new ItemStack(Core.registry.silver_ingot));
            } else {
    ...
    void loadWater() {
        ...
        for (FluidContainerData container : FluidContainerRegistry.getRegisteredFluidContainerData()) {
            ...
            if (h2o.isFluidEqual(liq) && liq.amount == FluidContainerRegistry.BUCKET_VOLUME
                && container.filledContainer != null) {
    ...
    public void addDictOres() {
        ...
        for (OreType type : ItemOreProcessing.OreType.values()) {
            ...
            if (oreList == null || !oreList.iterator()
                .hasNext()) {
                continue;
            }
            for (ItemStack ore : oreList) {
                ItemStack smeltsTo = FurnaceRecipes.smelting()
                    .getSmeltingResult(ore);
                if (smeltsTo == null) {
                    if (ingotClass == null) {
                        break;
