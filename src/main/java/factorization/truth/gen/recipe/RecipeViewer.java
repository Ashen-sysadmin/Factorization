...
public class RecipeViewer implements IDocGenerator, IObjectWriter<Object> {
    ...
    public void process(ITypesetter out, String arg) throws TruthError {
        ...
        if (arg == null || arg.equalsIgnoreCase("categories") || arg.isEmpty()) {
            ...
            if (!arg.equalsIgnoreCase("all")) {
                ...
                matchers = DocumentationModule.getNameItemCache()
                    .get(arg);
    ...
    int writeRecipes(ITypesetter out, ItemStack matching, boolean mustBeResult, String categoryName,
        ArrayList<ArrayList> recipes, HashSet<ArrayList> previously_found) throws TruthError {
        int got = 0;
        if (matching == null) {
    ...
    static void addRecipeCategory(String msg)
        throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
        String[] cmd = msg.split("\\|");
        String key = cmd[0];
        String className = cmd[1];
        String fieldName = cmd[2];
        Class kl = RecipeViewer.class.getClassLoader()
            .loadClass(className);
        ...
        if (obj instanceof Iterable) {
            ...
            Core.logWarning(
                "Unable to load recipe list provided by IMC message, obtained object is neither Iterable nor Map: "
                    + msg);
        }
    }
