...
public class InspirationManager {
    ...
    static final StatBase lastArtifact = new StatBase(
        "factorization.artifact.last",
        new ChatComponentTranslation("factorization.artifact.last.name")).registerStat();
    static final StatBase beenNotified = new StatBase(
        "factorization.artifact.notified",
        new ChatComponentTranslation("factorization.artifact.notify.name")).registerStat();
    ...
    static int today() {
        ...
        if (DEBUG) {
            return cal.get(Calendar.DAY_OF_YEAR) * 60 * 60 * 365 + cal.get(Calendar.HOUR) * 60 * 60
    ...
    public void poke(EntityPlayer player, boolean isLogin) {
        ...
        if (lastNoticeSent != today && isLogin) {
            player.addChatMessage(
                new ChatComponentTranslation("factorization.artifact.canstillcreate").setChatStyle(aqua));
            update = true;
        }
        if (update) {
    ...
    public static void makeArtifact(EntityPlayer player, ItemStack artifact) {
        ...
        IChatComponent msg = new ChatComponentTranslation(
            ...
            name,
            artifactName,
            new ChatComponentTranslation(toolName));
        msg = msg.setChatStyle(aqua);
        MinecraftServer.getServer()
            .getConfigurationManager()
            .sendChatMsgImpl(msg, false);
        resetArtifactDelay(player);
        for (Object obj : MinecraftServer.getServer()
            .getConfigurationManager().playerEntityList) {
            if (!(obj instanceof EntityPlayer)) continue;
