...
public class Quaternion implements IDataSerializable {
    ...
    public IDataSerializable serialize(String name_prefix, DataHelper data) throws IOException {
        w = data.asSameShare(name_prefix + "w")
            .putDouble(w);
        x = data.asSameShare(name_prefix + "x")
            .putDouble(x);
        y = data.asSameShare(name_prefix + "y")
            .putDouble(y);
        z = data.asSameShare(name_prefix + "z")
            .putDouble(z);
        return this;
    }
    ...
    private static Quaternion[] quat_cache = new Quaternion[25 /*
                                                                * FzOrientation.values().length recursive reference,
                                                                * bleh
                                                                */];
    ...
    public static Quaternion fromOrientation(final FzOrientation orient) {
        ...
        switch (orient.facing) {
            ...
            default:
                return quat_cache[ord] = new Quaternion(); // Won't happen
        }
    ...
    /**
     * When this Quaternion is going to be interpolated to other, it can be interpolated either the long way around, or
     * the short way.
