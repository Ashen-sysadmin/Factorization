...
public class EntityFXSteam extends EntityFX {
    ...
    @Override
    public void renderParticle(Tessellator tess, float partial, float dx, float dy, float dz, float xChange,
        float yChange) {
        ...
        tess.addVertexWithUV(
            (double) (x - dx * scale - xChange * scale),
            (double) (y - dy * scale),
            (double) (z - dz * scale - yChange * scale),
            (double) u1,
            (double) v1);
        tess.addVertexWithUV(
            (double) (x - dx * scale + xChange * scale),
