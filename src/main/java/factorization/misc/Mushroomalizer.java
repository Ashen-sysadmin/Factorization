...
public class Mushroomalizer {
    ...
    public void tickEnd(ServerTickEvent tick) {
        ...
        ForgeDirection[] HORIZONTALS = new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.SOUTH,
            ForgeDirection.EAST, ForgeDirection.WEST };
        for (PlayerInteractEvent event : shroomQueue) {
            if (event.isCanceled()) continue;
            Coord at = new Coord(event.entityPlayer.worldObj, event.x, event.y, event.z)
                .add(ForgeDirection.getOrientation(event.face));
