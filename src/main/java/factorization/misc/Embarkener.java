...
public class Embarkener {
    ...
    boolean isWoodish(Block block) {
        ...
        return block.getMaterial() == Material.wood && block instanceof BlockLog
            && block.getRenderType() == wood_rendertype;
    }
    ...
    void addLogBarkRecipes() {
        ...
        Core.logInfo(
            "Added %s 'barking' recipes for blocks that look sort of like wood logs; this can be disabled in the config file.",
            count);
    }
    ...
    static class EmbarkenEvent {
        ...
        public EmbarkenEvent(Coord target, int orig_stacksize, ItemStack stack, EntityPlayer player,
            BlockLog expectedBlock) {
            this.target = target;
