...
public class LagssieWatchDog implements Runnable {
    ...
    private static final String[] sickeningly_cute_lag_detected_messages = new String[] {
        "Woof! Something is being slow!", "Bark Bark! There's some lag! Bark bark!", "Pant Pant! Lag! Whiiinnee",
        "Whiine! Lag!", "Bark! Lag! Bark! Lag!", "Woof! Why is minecraft being so laggy!", "Yip! Lagfest!",
        "Bark! Minecraft is running slowly! It must be your computer! Bark!", "Arf! Why's it freezing up!",
        "Arf arf! Minecraft fell down the well!", "Bark! *I'm* the one who's supposed to play dead!" }; // This is what
                                                                                                        // makes this
                                                                                                        // really
                                                                                                        // useful.
    ...
    public void run() {
        ...
        log(
            "You can change how often I check for freezups with the command /f watchdog <waitInterval>; I also have configuration options!");
        int last_tick = 0;
        ...
        while (true) {
            ...
            if (ticks == last_tick) {
                if (had_good_tick) {
                    if (be_cute) {
                        log(
                            sickeningly_cute_lag_detected_messages[rng
                                .nextInt(sickeningly_cute_lag_detected_messages.length)]);
                    }
                    had_good_tick = false;
                } else {
                    log("");
