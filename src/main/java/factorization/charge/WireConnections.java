...
public class WireConnections {
    ...
    long edges = 0; // bitmask of edges: edges on lower face NESW, upper face NESW, middle edges starting from NE and
                    // going clockwise
    ...
    void calculate() {
        // ******************************************************************************************************************************
        // NOTE: Be *very* careful with this code, it's a huge PITA. git commit & check the wire testing area before &
        // after any changes.
        // ******************************************************************************************************************************
    ...
    static WireRenderingCube base_face_side = base_face.copy()
        .rotate(1, 0, 0, 90)
        .normalize();
    static WireRenderingCube face_map[] = { base_face.copy(), base_face.rotate(1, 0, 0, 180)
        .normalize(),
        base_face_side.rotate(0, 1, 0, 90 * 0)
            .normalize(),
        base_face_side.rotate(0, 1, 0, 90 * 2)
            .normalize(),
        base_face_side.rotate(0, 1, 0, 90 * 1)
            .normalize(),
        base_face_side.rotate(0, 1, 0, 90 * 3)
            .normalize(), };
    static WireRenderingCube bottom_edge = cube(new VectorUV(w, h, h), new VectorUV(0, -8 + h, 8 - h)),
        top_edge = bottom_edge.copy()
            .rotate(1, 0, 0, 180)
            .normalize(),
        side_edge = bottom_edge.copy()
            .rotate(0, 0, 1, 90)
            .normalize();
    static WireRenderingCube edge_map[] = { bottom_edge.rotate(0, 1, 0, 90 * 2)
        .normalize(),
        bottom_edge.rotate(0, 1, 0, 90 * 1)
            .normalize(),
        bottom_edge.rotate(0, 1, 0, 90 * 0)
            .normalize(),
        bottom_edge.rotate(0, 1, 0, 90 * 3)
            .normalize(),
        top_edge.rotate(0, 1, 0, 90 * 0)
            .normalize(),
        top_edge.rotate(0, 1, 0, 90 * 3)
            .normalize(),
        top_edge.rotate(0, 1, 0, 90 * 2)
            .normalize(),
        top_edge.rotate(0, 1, 0, 90 * 1)
            .normalize(),
        side_edge.rotate(0, 1, 0, 90 * 1)
            .normalize(),
        side_edge.rotate(0, 1, 0, 90 * 0)
            .normalize(),
        side_edge.rotate(0, 1, 0, 90 * 3)
            .normalize(),
        side_edge.rotate(0, 1, 0, 90 * 2)
            .normalize(), };
