...
public class TileEntityDayBarrelRenderer extends TileEntitySpecialRenderer {
    ...
    void doDraw(TileEntityDayBarrel barrel, ItemStack is) {
        ...
        if (barrel.type == Type.HOPPING) {
            double time = barrel.getWorldObj()
                .getTotalWorldTime();
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float partial) {
        ...
        try {
            ...
            if (FzConfig.render_barrel_use_displaylists && barrel.type != Type.HOPPING
                && barrel.should_use_display_list
                && barrel != FactoryType.DAYBARREL.getRepresentative()) {
                if (barrel.warmup_time < 20) {
                    ...
                    GL11.glEndList();
                    if (RenderUtil
                        .checkGLError("FZ -- after barrel display list; does the item have an advanced renderer?")) {
                        Core.logSevere("The item is: " + is);
                        Core.logSevere("At: " + new Coord(barrel));
                        barrel.should_use_display_list = false;
                        doDraw(barrel, is);
    ...
    final String[] fontIdx = new String[] { "0123", "4567", "89*+", "i!  " // 'i' stands in for ∞, '!' stands in for
                                                                           // '!!'
    };
    ...
    boolean renderItemCount(ItemStack item, TileEntityDayBarrel barrel) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            tess.addVertexWithUV(IX + char_width, 0, 0, u + (x + 1) * du, v + y * dv);
            tess.addVertexWithUV(IX, 0, 0, u + x * du, v + y * dv);
            tess.addVertexWithUV(IX, char_height, 0, u + x * du, v + (y + dy) * dv);
            tess.addVertexWithUV(IX + char_width, char_height, 0, u + (x + 1) * du, v + (y + dy) * dv);
    ...
    public void handleRenderItem(ItemStack is, TileEntityDayBarrel barrel, boolean hasLabel) {
        ...
        boolean useEntityRenderer = is.getItem()
