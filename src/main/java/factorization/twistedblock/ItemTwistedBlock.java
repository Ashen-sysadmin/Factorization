...
public class ItemTwistedBlock extends ItemBlockProxy {
    ...
    final int channel = Hammer.instance.hammerInfo
        .makeChannelFor(Core.name, "twistedBlocks", 10, 64, "Allows placement of blocks at angles");
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        for (DeltaCapability allowed : new DeltaCapability[] { DeltaCapability.BLOCK_MINE, DeltaCapability.BLOCK_PLACE,
            DeltaCapability.INTERACT, DeltaCapability.COLLIDE, DeltaCapability.ROTATE, DeltaCapability.DIE_WHEN_EMPTY,
            DeltaCapability.DRAG, DeltaCapability.ENTITY_PHYSICS }) {
            idc.permit(allowed);
        }
        ForgeDirection axis = ForgeDirection.getOrientation(side);
        double amount = Math.toRadians(45);
        idc.getCenter()
            .setIdMd(darkIron, darkIronMd, true);
        idc.posX = at.x + 0.5;
        idc.posY = at.y + 0.5;
        idc.posZ = at.z + 0.5;

        Vec3 center = idc.getRotationalCenterOffset()
        ...
        at.add(ForgeDirection.getOrientation(side))
            .spawnItem(Core.registry.dark_iron_sprocket.copy());
